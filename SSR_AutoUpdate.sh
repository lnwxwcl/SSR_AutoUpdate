...
get_allssr(){
	...
	echo "$(date "+%Y-%m-%d %H:%M:%S") 筛选完毕，节点编号： $port，其ping值：$ping_ttl ms" >> /tmp/clingLogs.txt
	if [ $ping_ttl != 99999 ] && [ $port != 0 ]
		...
		echo "$(date "+%Y-%m-%d %H:%M:%S") 【UPDATE】设置新节点成功，节点编号： $port" >> /tmp/clingLogs.txt
	fi
}
...
check=$?
if [ "$check" == "0" ]; then
	echo "$(date "+%Y-%m-%d %H:%M:%S") 【Success】Check YouTube Proxy Pass." >> /tmp/clingLogs.txt
	break
else
	echo "$(date "+%Y-%m-%d %H:%M:%S") 【ERROR】Check YouTube Proxy Fail. 触发ping全部节点" >> /tmp/clingLogs.txt
